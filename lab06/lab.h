...
inline Vec3 norm(const Vec3&v){float L=vlen(v);return L?Vec3{v.x/L,v.y/L,v.z/L}:Vec3{0,0,0};}
...
inline Mat4 operator*(const Mat4&A,const Mat4&B){Mat4 C{};for(int r=0;r<4;++r)for(int c=0;c<4;++c){float s=0.f;for(int k=0;k<4;++k)s+=A.m[r][k]*B.m[k][c];C.m[r][c]=s;}return C;}
inline Vec4 operator*(const Vec4&v,const Mat4&M){Vec4 r{};r.x=v.x*M.m[0][0]+v.y*M.m[1][0]+v.z*M.m[2][0]+v.w*M.m[3][0];r.y=v.x*M.m[0][1]+v.y*M.m[1][1]+v.z*M.m[2][1]+v.w*M.m[3][1];r.z=v.x*M.m[0][2]+v.y*M.m[1][2]+v.z*M.m[2][2]+v.w*M.m[3][2];r.w=v.x*M.m[0][3]+v.y*M.m[1][3]+v.z*M.m[2][3]+v.w*M.m[3][3];return r;}
inline Vec3 xform(const Vec3&p,const Mat4&M){Vec4 v{p.x,p.y,p.z,1.f};Vec4 r=v*M;return std::abs(r.w)<1e-6f?Vec3{r.x,r.y,r.z}:Vec3{r.x/r.w,r.y/r.w,r.z/r.w};}

struct Vertex{float x{},y{},z{};};
struct Face{vector<int> idx;};
struct Mesh{vector<Vertex> V; vector<Face> F;};

inline Vec3 centroid(const Mesh&m){Vec3 c{0,0,0};if(m.V.empty())return c;for(auto&p:m.V){c.x+=p.x;c.y+=p.y;c.z+=p.z;}float inv=1.f/(float)m.V.size();return {c.x*inv,c.y*inv,c.z*inv};}

inline Mesh makeCube(float s=1.f){Mesh P;float a=s;P.V={{-a,-a,-a},{a,-a,-a},{a,a,-a},{-a,a,-a},{-a,-a,a},{a,-a,a},{a,a,a},{-a,a,a}};P.F={{{0,1,2,3}},{{4,5,6,7}},{{0,1,5,4}},{{3,2,6,7}},{{1,2,6,5}},{{0,3,7,4}}};return P;}
inline Mesh makeTetra(float s=1.f){Mesh P;float a=s;P.V={{-a,a,-a},{a,-a,-a},{a,a,a},{-a,-a,a}};P.F={{{0,1,2}},{{0,1,3}},{{0,2,3}},{{1,2,3}}};return P;}
inline Mesh makeOcta(float s=1.f){Mesh P;float a=s;P.V={{0,0,a},{0,0,-a},{0,a,0},{0,-a,0},{a,0,0},{-a,0,0}};P.F={{{0,2,4}},{{0,4,3}},{{0,3,5}},{{0,5,2}},{{1,4,2}},{{1,3,4}},{{1,5,3}},{{1,2,5}}};return P;}

inline Mesh makeIcosa(float s=1.f){
    const float phi=(1.f+std::sqrt(5.f))*0.5f;
    vector<Vec3> W={{0,-1,-phi},{0,-1,phi},{0,1,-phi},{0,1,phi},{-1,-phi,0},{-1,phi,0},{1,-phi,0},{1,phi,0},{-phi,0,-1},{phi,0,-1},{-phi,0,1},{phi,0,1}};
    float maxr=0.f;for(auto&p:W)maxr=std::max(maxr,vlen(p));float k=s/maxr;
    Mesh P;for(auto&p:W)P.V.push_back({p.x*k,p.y*k,p.z*k});
    P.F={
            {{0,8,2}},{{0,2,9}},{{0,9,6}},{{0,6,1}},{{0,1,8}},
            {{1,6,11}},{{1,11,10}},{{1,10,8}},{{2,8,5}},{{2,5,7}},
            {{2,7,9}},{{3,5,8}},{{3,11,5}},{{3,10,11}},{{3,4,10}},
            {{3,8,4}},{{4,8,10}},{{5,11,7}},{{6,9,11}},{{7,11,9}}
    };
    return P;
}
...
inline Mesh makeDodeca(float s=1.f){
    ...
    vector<Vec3> centers;centers.reserve(ico.F.size());
    for(auto&f:ico.F){Vec3 c{0,0,0};for(int i:f.idx)c=c+Vec3{ico.V[i].x,ico.V[i].y,ico.V[i].z};c=c*(1.f/(float)f.idx.size());centers.push_back(norm(c));}
    for(auto&c:centers)c=c*s;
    Mesh dode;for(auto&c:centers)dode.V.push_back({c.x,c.y,c.z});
    ...
    for(int vi=0;vi<(int)ico.V.size();++vi){
        auto&fs=inc[vi];if((int)fs.size()!=5)continue;
        ...
        struct It{int f;float ang;};vector<It> items;items.reserve(5);
        for(int fidx:fs){Vec3 C=centers[fidx];Vec3 v=C-Nv*dot(C,Nv);float x=dot(v,e1),y=dot(v,e2);items.push_back({fidx,std::atan2(y,x)});}
        std::sort(items.begin(),items.end(),[](const It&a,const It&b){return a.ang<b.ang;});
        Face pent;for(auto&t:items)pent.idx.push_back(t.f);dode.F.push_back(std::move(pent));
    }
    return dode;
}
...
struct Projector{
    ...
    bool project(const Vec3&pw,int&X,int&Y)const{
        if(perspective){
            float denom=f+pw.z; if(denom<=1e-3f) return false;
...
static void drawAxes(const Projector&proj,float len=250.f){
    ...
    if(proj.project(O,ox,oy)&&proj.project(X,xx,xy))dl->AddLine(ImVec2(ox,oy),ImVec2(xx,xy),IM_COL32(255,0,0,255),2.f);
    if(proj.project(O,ox,oy)&&proj.project(Y,yx,yy))dl->AddLine(ImVec2(ox,oy),ImVec2(yx,yy),IM_COL32(0,200,0,255),2.f);
    if(proj.project(O,ox,oy)&&proj.project(Z,zx,zy))dl->AddLine(ImVec2(ox,oy),ImVec2(zx,zy),IM_COL32(0,128,255,255),2.f);
...
static void drawWireImGui(const Mesh&base,const Mat4&model,const Projector&proj,ImU32 color=IM_COL32(20,20,20,255),float thick=1.8f){
    ImDrawList* dl=ImGui::GetBackgroundDrawList();
    for(const auto& f: base.F){
        for(size_t i=0;i<f.idx.size();++i){
            ...
            int x0,y0,x1,y1; if(proj.project(a,x0,y0)&&proj.project(b,x1,y1)) dl->AddLine(ImVec2((float)x0,(float)y0),ImVec2((float)x1,(float)y1),color,thick);
        }
    }
}
...
static void applyKeyOps(GLFWwindow* w,AppState& S){
    ...
    if(down(GLFW_KEY_1)){S.kind=PolyKind::Tetra;S.base=makeTetra(150.f);S.modelMat=Mat4::I();}
    if(down(GLFW_KEY_2)){S.kind=PolyKind::Cube; S.base=makeCube (150.f);S.modelMat=Mat4::I();}
    if(down(GLFW_KEY_3)){S.kind=PolyKind::Octa; S.base=makeOcta (150.f);S.modelMat=Mat4::I();}
    if(down(GLFW_KEY_4)){S.kind=PolyKind::Ico;  S.base=makeIcosa(150.f);S.modelMat=Mat4::I();}
    if(down(GLFW_KEY_5)){S.kind=PolyKind::Dode; S.base=makeDodeca(150.f);S.modelMat=Mat4::I();}
    ...
    if(down(GLFW_KEY_C)){
        switch(S.kind){
            case PolyKind::Tetra: S.base=makeTetra(150.f); break;
            case PolyKind::Cube:  S.base=makeCube (150.f); break;
            case PolyKind::Octa:  S.base=makeOcta (150.f); break;
            case PolyKind::Ico:   S.base=makeIcosa(150.f); break;
            case PolyKind::Dode:  S.base=makeDodeca(150.f); break;
        }
        S.modelMat=Mat4::I();
...
int run(){
    ...
    while(!glfwWindowShouldClose(win)){
        ...
        if(ImGui::Combo("Polyhedron",&polyIdx,items,IM_ARRAYSIZE(items))){
            S.modelMat=Mat4::I();
            if(polyIdx==0){S.kind=PolyKind::Tetra;S.base=makeTetra(150.f);}
            if(polyIdx==1){S.kind=PolyKind::Cube; S.base=makeCube (150.f);}
            if(polyIdx==2){S.kind=PolyKind::Octa; S.base=makeOcta (150.f);}
            if(polyIdx==3){S.kind=PolyKind::Ico;  S.base=makeIcosa(150.f);}
            if(polyIdx==4){S.kind=PolyKind::Dode; S.base=makeDodeca(150.f);}
        }
        if(ImGui::Checkbox("Perspective",&persp)) S.proj.perspective=persp;
        ImGui::Checkbox("Show axes",&showAxes);
        ImGui::SliderFloat("Scale",&S.proj.scale,20.f,400.f);
        if(!S.proj.perspective){ImGui::SliderFloat("Axon X",&S.proj.ax,0.f,90.f);ImGui::SliderFloat("Axon Y",&S.proj.ay,0.f,90.f);} else {ImGui::SliderFloat("Focus f",&S.proj.f,100.f,2000.f);}
        ImGui::SeparatorText("Rotate around center");
        if(ImGui::Button("X +5")){Vec3 Cw=worldCenter(S.base,S.modelMat);worldRotateAroundAxisThrough(S,Cw,{1,0,0},+5.f);}
        ImGui::SameLine();
        if(ImGui::Button("Y +5")){Vec3 Cw=worldCenter(S.base,S.modelMat);worldRotateAroundAxisThrough(S,Cw,{0,1,0},+5.f);}
        ImGui::SameLine();
        if(ImGui::Button("Z +5")){Vec3 Cw=worldCenter(S.base,S.modelMat);worldRotateAroundAxisThrough(S,Cw,{0,0,1},+5.f);}
        ImGui::SeparatorText("Reflect");
        if(ImGui::Button("XY")) worldReflectPlane(S,Mat4::RefXY()); ImGui::SameLine();
        ...
        if(ImGui::Button("Reset [C]")){
            ...
            S.modelMat=Mat4::I();S.proj.perspective=true;S.proj.ax=35.264f;S.proj.ay=45.f;S.proj.scale=160.f;persp=true;
        }
        ImGui::End();
